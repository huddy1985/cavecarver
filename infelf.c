...
int main(int argc, char* argv[])
{
    ...
    {
        if (!strcmp(argv[i], "-dump"   )) OPT_DUMP++;
        else if (!strcmp(argv[i], "-relref-")) df_flags &= ~DF_RELREF;
        else if (!strcmp(argv[i], "-jmptab-")) df_flags &= ~DF_JMPTAB;
        else if (!strcmp(argv[i], "-func-"  )) df_flags &= ~DF_FUNC;
        else if (!strcmp(argv[i], "-sym-"   )) df_flags &= ~DF_SYM;
        else if (!strcmp(argv[i], "-got-"   )) df_flags &= ~DF_GOT;
        else if (!strcmp(argv[i], "-nop-"   )) in_flags &= ~IN_KILLNOP;
        else if (!strcmp(argv[i], "-bf"     )) in_flags |= IN_BIGFIRST;
        else if ((!strcmp(argv[i], "-outmap" ))&&(i+1<argc)) map     = argv[++i];
        else if ((!strcmp(argv[i], "-outtext"))&&(i+1<argc)) text    = argv[++i];
        else if ((!strcmp(argv[i], "-out"    ))&&(i+1<argc)) outfile = argv[++i];
        else if ((!strcmp(argv[i], "-snippet"))&&(i+1<argc)) snippet = argv[++i];
        else if ((!strcmp(argv[i], "-hookaddr"))&&(i+1<argc))
        {
            i++;
            hook_isva = argv[i][0]=='.';
            sscanf(&argv[i][hook_isva], "%X", &hook_offs);
        } else if (!strcmp(argv[i], "-hookentry")) hook_isentry++;
        else if ((!strcmp(argv[i], "-hookfunc"))&&(i+1<argc)) hook_func = argv[++i];
        else if ((!strcmp(argv[i], "-hooksign"))&&(i+1<argc)) hook_sign = argv[++i];
        else if (infile == 0)
            infile = argv[i];
        else
            help();
    ...
    if (X->payload( df_flags ) != ERR_SUCCESS)
